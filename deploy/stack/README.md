...
- verifies `/var/lib/open-review` exists and is writable before starting containers
...
- `daily_audit` uses a run-scoped sandbox experiment directory for local compile/benchmark work; this stack is intended for local C/C++ experiments and local benchmark workflows, not whole-project builds.
